...
-- -----------------------------------------------------
-- Table `organicos_bd`.`producto_usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `organicos_bd`.`producto_usuario` (
  `id_producto` INT NOT NULL,
  `id_usuario` INT NOT NULL,
  PRIMARY KEY (`id_producto`, `id_usuario`),
  INDEX `C_FK_C_PC_idx` (`id_usuario` ASC) VISIBLE,
  ...
  CONSTRAINT `C_FK_C_PC`
    ...
    REFERENCES `organicos_bd`.`usuario` (`idusuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
